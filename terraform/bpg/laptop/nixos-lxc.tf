...
resource "terraform_data" "bootstrap_ct" {
  ...
  provisioner "file" {
    source      = var.ct_bootstrap[count.index]
    destination = "/etc/nixos/${basename(var.ct_bootstrap[count.index])}"
  }
  ...
  lifecycle {
    precondition {
      condition     = try(var.ct_bootstrap[count.index], null) != null
      error_message = "Bootstrap cannot be executed without a nix config."
    }
    precondition {
